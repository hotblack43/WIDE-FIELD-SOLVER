...
_DEEP_STAGES2 = ((1.0, 8.0, 3.0), (1.0, 8.5, 3.0))


def _validated_limit2(limit):
    limit = float(limit)
    if limit not in SUPPORTED_LIMITS2:
        raise ValueError(
            f"Unsupported limiting magnitude {limit:g}; supported values are "
            + ", ".join(f"{value:g}" for value in SUPPORTED_LIMITS2)
        )
    return limit


def association_stages2(limit):
    """Return the v2 progressive association schedule for one candidate depth."""
    limit = _validated_limit2(limit)
    return _BASE_STAGES2 + tuple(stage for stage in _DEEP_STAGES2
                                if stage[1] <= limit)
...
def run2(image_path, output, catalog_path, *, magnitude_limit, label_count=40,
         names_cache=None, offline=False, overwrite=True, observation_time=None,
         latitude=None, longitude=None, elevation_m=0., pressure_hpa=None,
         temperature_c=10., relative_humidity=.5, extinction_mag_per_airmass=None):
    magnitude_limit = _validated_limit2(magnitude_limit)
    ...
    if not len(finite_mags) or finite_mags.max() + 1e-9 < magnitude_limit:
        available = finite_mags.max() if len(finite_mags) else float('nan')
        raise ValueError(
            f"Catalogue only reaches magnitude {available:g}; "
            f"candidate {magnitude_limit:g} requires a deeper catalogue"
        )
    centre = (np.array(shape[::-1])-1)/2
    radius = np.linalg.norm(xy[train]-centre, axis=1)
    stages = []
    # V2 preserves the working bright-star progression and adds only requested depths.
    for radial_fraction, mag, gate in association_stages2(magnitude_limit):
        use = train[radius < radial_fraction*min(shape)]
        cat_use = catalogue_indices2(mags, mag)
    ...
    final_cat_use = catalogue_indices2(mags, magnitude_limit)
    train_i, limited_j = associate(camera, xy[train], sky[final_cat_use], 3.)
    train_j = final_cat_use[limited_j]
    if len(train_i) < 20:
    ...
    result = dict(status='point_star_fit_converged' if accepted else 'point_star_fit_not_converged',
        ...
        solver_version=2, magnitude_limit=magnitude_limit,
        catalogue_rows_at_limit=len(final_cat_use),
...
def main():
    ...
    parser.add_argument('--catalog', type=Path,
                        default=Path(__file__).resolve().parent/'data/stars_tycho2_mag85_v2.csv')
    parser.add_argument('--magnitude-limit', type=float, choices=SUPPORTED_LIMITS2, default=7.5,
                        help='V2 catalogue limit to fit (default: 7.5)')
    ...
    if args.annotations_from:
        ...
        return
    result = run2(args.image, args.output, args.catalog,
                 magnitude_limit=args.magnitude_limit, label_count=args.labels,
                 names_cache=args.names_cache, offline=args.offline,
