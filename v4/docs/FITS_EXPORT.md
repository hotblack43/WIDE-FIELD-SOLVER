...
`go4.sh` writes `solution.fits` after the scientific fits are fixed. The file
...
```sh
./v4/view_fits.sh /path/to/analysis/solution.fits
```
...
```sh
uv run --project v4 --frozen python v4/point_star_fits.py export /path/to/analysis
```
