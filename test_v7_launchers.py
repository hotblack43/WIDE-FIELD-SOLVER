"""Exercise packaged v7 dispatch without a worktree or scientific dependencies."""
import ast
import json
...
LAUNCHERS = ('go7.sh', 'go_v0.7.0.sh')
VERSION = '0.7.0'


class V7LauncherTests(unittest.TestCase):
    def test_package_version_interfaces_agree(self):
        project = tomllib.loads((ROOT/'v7/pyproject.toml').read_text())
        lock = tomllib.loads((ROOT/'v7/uv.lock').read_text())
        ...
        module = ast.parse((ROOT/'v7/point_star_barghini.py').read_text())
    ...
    def make_package(self, root, version=VERSION):
        (root/'v7/data').mkdir(parents=True)
        for name in LAUNCHERS + ('v7/run.sh',):
            destination = root/name
            destination.parent.mkdir(parents=True, exist_ok=True)
            shutil.copy2(ROOT/name, destination)
        (root/'v7/data/stars_gaia_dr3_g75.csv').write_text('fixture')
        stub = root/'v7/analyse.sh'
    ...
    def test_both_launchers_run_local_v7_and_preserve_each_output(self):
        with tempfile.TemporaryDirectory(prefix='solver package ') as tmp:
            ...
            for launcher in LAUNCHERS:
                for _ in range(2):
                    ...
                    self.assertIn('Solver: Wide-field solver 0.7.0, Gaia DR3', completed.stdout)
            runs = sorted((root/'results/runs').iterdir())
            self.assertEqual(len(runs), 4)
            for run in runs:
                self.assertIn('-v0.7.0-', run.name)
                args = json.loads((run/'analysis/args.json').read_text())
                self.assertEqual(args, [str(image), '--catalog',
                                       str(root/'v7/data/stars_gaia_dr3_g75.csv'),
