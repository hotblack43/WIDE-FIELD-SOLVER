...
def measure_photometry(image_path, solution, result, refraction):
    """Measure every detection, then fit catalogue-relative stellar extinction."""
    solution = Path(solution)
    ...
    if set(scientific.planes) == {'R', 'G1', 'G2', 'B'}:
        channel_masks = {
            ...
            'G': scientific.plane_saturated_masks['G1'] | scientific.plane_saturated_masks['G2'],
    ...
    measured_channels = [*'RGB', *native_extra]
    measurement_suffixes = [
        'count_rate_adu_per_s', 'saturated', 'measurement_method',
        'saturated_pixels_in_aperture', 'wing_fit_status',
        'wing_fit_total_counts_adu', 'wing_fit_count_rate_adu_per_s',
        'wing_fit_x_px', 'wing_fit_y_px', 'wing_fit_rms_adu',
        'wing_fit_unsaturated_pixels']
    source_fields = [
    ...
    for name in measured_channels:
        source_fields.extend([f'{name}_flux', *[f'{name}_{suffix}' for suffix in measurement_suffixes]])
    source_records = []
    for identity, detection in detections.items():
        coordinate = coordinates.get(identity)
        position = coordinate or detection
        x, y = _float(position, 'x_px'), _float(position, 'y_px')
        sigma = max(1., _float(detection, 'major_sigma_px', 1.5))
        aperture_radius = min(9., max(3., 2.5*sigma))
        record = dict(
            detection_id=identity, x_px=x, y_px=y,
            star_id=(coordinate['star_id'] if coordinate else ''),
            identified_as_star=bool(coordinate),
            source_class=detection['source_class'], saturated=detection['saturated'],
            saturation_known=detection.get('saturation_known', scientific.provenance()['saturation_known']),
        ...
        for channel in measured_channels:
            measured = measure_channel(
                channels_data[channel], scientific.valid_mask, channel_masks[channel],
                x=x, y=y, aperture_radius=aperture_radius,
                exposure_seconds=exposure_seconds)
            record[f'{channel}_flux'] = measured['aperture_counts_adu']
            for suffix in measurement_suffixes:
                record[f'{channel}_{suffix}'] = measured.get(suffix, float('nan'))
        source_records.append(record)
    with (solution/'source_photometry.csv').open('w', newline='') as handle:
        writer = csv.DictWriter(handle, fieldnames=source_fields)
        writer.writeheader()
        writer.writerows(source_records)
    source_by_id = {row['detection_id']: row for row in source_records}
    ...
    for name in measured_channels:
        fields.extend([f'{name}_{suffix}' for suffix in measurement_suffixes])
    for name in native_extra:
        fields.extend([f'{name}_flux', f'{name}_mag'])
    records = []
    camera = _camera_from_result(result)
    for identity, coordinate in coordinates.items():
        ...
        mags = np.array([
            _machine_magnitude_from_rate(flux, source_record['exposure_seconds'])
            for flux in fluxes])
        ...
        record = dict(detection_id=identity, star_id=coordinate['star_id'],
                      ...
                      saturated_channels=detection.get('saturated_channels', ''),
                      exposure_seconds=source_record['exposure_seconds'],
                      exposure_status=source_record['exposure_status'],
                      exposure_source=source_record['exposure_source'],
        ...
        for channel, flux, magnitude in zip('RGB', fluxes, mags):
            ...
            for suffix in measurement_suffixes:
                record[f'{channel}_{suffix}'] = source_record[f'{channel}_{suffix}']
    ...
    summary = dict(status='instrumental_photometry_measured', measured_stars=len(records),
                   measured_sources=len(source_records),
                   ...
                   calibrated=False,
                   count_rate_unit='ADU/s', exposure=exposure,
