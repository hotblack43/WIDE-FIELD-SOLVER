# Gaia v0.11.0 joint stellar/planetary wide-field solver

V11 preserves v10 as an independent parent and adds a common-camera stellar and
planetary epoch profile, full-range discovery and soft ephemeris brightness.
See [joint epoch status and numerical changes](docs/V11_JOINT_EPOCH.md).

For installation, choosing a solver and running images, use the
[shared guide for go.sh through go10.sh](../README.md).

Run `./go11.sh /path/to/image.jpg` from the
repository root. This directory contains v0.11.0 and its local Python modules,
Gaia catalogue, provenance, dependency lockfile, tests and scientific status.
No development worktree is required.

Every `go11.sh` analysis automatically
