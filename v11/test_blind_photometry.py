...
class BlindPhotometryTests(unittest.TestCase):
    def test_all_detections_get_count_rates_before_identity_assignment(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            (root/'dots').mkdir()
            planes = np.full((3, 80, 80), 100, dtype=np.int16)
            planes[:, 19:22, 19:22] = 500
            planes[0, 49:52, 49:52] = 900
            planes[1:, 49:52, 49:52] = 4095
            source = root/'sources.fits'
            hdu = fits.PrimaryHDU(planes)
            hdu.header['EXPTIME'] = 2.
            hdu.header['SATURATE'] = 4095
            hdu.writeto(source)
            loaded = load_scientific_image(source)
            (root/'dots/input_image.json').write_text(json.dumps(loaded.provenance()))
            coordinates = [dict(detection_id='1', star_id='gaia-1', x_px=20, y_px=20,
                                magnitude=5., residual_px=.1)]
            detections = [
                dict(detection_id='1', x_px=20, y_px=20, saturated='False',
                     saturation_known='True', saturated_channels='',
                     source_class='compact', major_sigma_px=1.),
                dict(detection_id='2', x_px=50, y_px=50, saturated='True',
                     saturation_known='True', saturated_channels='G;B',
                     source_class='broad_blob', major_sigma_px=2.)]
            for path, rows in ((root/'star_coordinates.csv', coordinates),
                               (root/'dots/star_candidates.csv', detections)):
                with path.open('w') as handle:
                    writer = csv.DictWriter(handle, fieldnames=list(rows[0]))
                    writer.writeheader(); writer.writerows(rows)
            camera = BarghiniCamera.initial((80, 80), 30, np.eye(3))
            unresolved = dict(status='not_identifiable', zenith_unit_vector=None,
                              reason='fixture', fitted_count=1)
            with patch('point_star_zenith.fit_photometric_zenith', return_value=unresolved):
                summary = measure_photometry(source, root, {'camera': camera.serialise()}, {})
        ...
        self.assertEqual(summary['measured_sources'], 2)
        self.assertEqual(len(sources), 2)
        self.assertEqual(len(stars), 1)
        self.assertEqual(sources[1]['identified_as_star'], 'False')
        self.assertAlmostEqual(float(sources[1]['R_count_rate_adu_per_s']),
                               float(sources[1]['R_flux'])/2.)
        self.assertEqual(sources[1]['R_measurement_method'], 'aperture')
        self.assertEqual(sources[1]['G_measurement_method'],
                         'saturated_aperture_lower_bound')
        self.assertEqual(sources[1]['exposure_source'], 'fits:PRIMARY:EXPTIME')
        self.assertAlmostEqual(float(stars[0]['R_mag']), -2.5*np.log10(1800.))
    ...
    def test_photometry_does_not_read_site_or_time(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            source = root/'warwick_20260101T000000Z.fits'
            ...
            hdu = fits.PrimaryHDU(image.transpose(2, 0, 1))
            hdu.header['EXPTIME'] = 1.
            hdu.header['SATURATE'] = 255
            hdu.writeto(source)
    ...
    def test_unsaturated_identified_sources_enter_fixed_sample_with_explicit_flags(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch('point_star_zenith.fit_photometric_zenith', return_value=unresolved) as fit:
                answer = measure_photometry(root/'source.fits', root, {'camera': camera.serialise()}, {})
    ...
    def test_default_planet_path_keeps_metadata_for_validation_only(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(search.call_args.kwargs['search_context']['planet_search_mode'],
                             'blind_global_metadata_validation')
