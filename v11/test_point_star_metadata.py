...
class ObservationTimeMetadataTests(unittest.TestCase):
    ...
    def test_bz2_fits_time_metadata_matches_uncompressed(self):
        from point_star_metadata import resolve_observation_time
        with tempfile.TemporaryDirectory() as directory:
            plain = Path(directory)/'plain.fits'
            header = fits.Header({'DATE-OBS': '2026-01-17T08:53:30', 'TIMESYS': 'UTC'})
            fits.PrimaryHDU(np.zeros((8, 8)), header).writeto(plain)
            expected = resolve_observation_time(plain)
            for suffix in ('.fits.bz2', '.fit.bz2', '.FTS.BZ2'):
                with self.subTest(suffix=suffix):
                    compressed = Path(directory)/('compressed'+suffix)
                    compressed.write_bytes(bz2.compress(plain.read_bytes()))
                    self.assertEqual(resolve_observation_time(compressed), expected)
    ...
    def test_metadata_date_is_validation_only_and_search_stays_global(self):
        ...
        self.assertEqual(context['planet_search_mode'], 'blind_global_metadata_validation')
        self.assertFalse(context['metadata_used'])
        self.assertEqual(context['epoch_limits'], (1850., 2036.))
        self.assertAlmostEqual(context['observation_time_metadata']['jd_tdb'],
                               Time('2018-09-16T00:13:55', scale='utc').tdb.jd, places=8)
