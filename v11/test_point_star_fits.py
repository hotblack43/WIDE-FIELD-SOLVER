...
class FitsExportTests(unittest.TestCase):
    def test_joint_fallback_fits_uses_candidate_identity_not_initial_global_matches(self):
        self.science['joint_epoch'] = dict(status='joint_epoch_ambiguous', adopted=False,
            best_index=0, candidates=[dict(epoch_tdb='2020', jd_tdb=2458849., planet_count=2,
                planet_rms_arcmin=2., matches=[dict(planet='Jupiter', detection_id=2,
                    measured_x_px=42., measured_y_px=43., predicted_x_px=43., predicted_y_px=44.)])])
        rows = self.api.collect_overlays(self.out, self.camera, self.science)
        planetary = [r for r in rows if 'planet' in r['kind']]
        self.assertEqual(len(planetary), 1)
        self.assertEqual(planetary[0]['label'], 'Jupiter (candidate)')
        self.assertEqual((planetary[0]['x'], planetary[0]['y']), (42., 43.))
    def setUp(self):
