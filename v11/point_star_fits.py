...
def collect_overlays(output, camera, science):
    ...
    if science.get('joint_epoch'):
        from point_star_joint_epoch import planet_product_view
        planets = planet_product_view(planets, science['joint_epoch'])
    for kind, group, xkey, ykey in [
        ('matched_planet', 'matches', 'measured_x_px', 'measured_y_px'),
        ('candidate_planet', 'candidate_matches', 'measured_x_px', 'measured_y_px'),
        ...
        for p in planets.get(group) or []:
            label = p['planet'] + (' (predicted)' if kind == 'predicted_planet' else
                    (' (candidate)' if kind == 'candidate_planet' or planets.get('status') in ('planet_epoch_ambiguous', 'conditional_planet_epoch') else ''))
...
def write_fits(image_path, output, result, science):
    ...
    try:
        ...
        c=result['camera']
        camera=BarghiniCamera.from_serialised(c)
        ...
        info=dict(camera=c,source_sha256=source_hash,
                  ...
                  sky_mask=mask_info,
                  planet_status=(science.get('joint_epoch') or science.get('planets') or {}).get('status'),
                  planet_epoch_tdb=(science.get('joint_epoch') or science.get('planets') or {}).get('best_candidate_epoch_tdb'),
