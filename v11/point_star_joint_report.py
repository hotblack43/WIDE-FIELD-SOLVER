...
def joint_figure(output, result, science):
    ...
    grid = fig.add_gridspec(3, 4, height_ratios=[1., 1.05, .85],
                          left=.06, right=.97, top=.81, bottom=.04, hspace=.6, wspace=.45)
    status = joint['status'].replace('_', ' ')
    fig.suptitle(f'V11: stars + planets fitted together\n{status.upper()}', fontsize=15, weight='bold', y=.98)
    fig.text(.5, .89, 'Joint camera ADOPTED' if joint.get('adopted') else
             'Candidate refits are diagnostic. The saved scientific camera remains STELLAR-ONLY.',
             ha='center', fontsize=11, color='#a00030' if not joint.get('adopted') else '#006020')
    ...
    if candidates:
        ...
        full.set_xlabel('Candidate epoch (Julian year TDB)')
    ...
    if best:
        ...
        local.set_xlabel('Hours relative to joint candidate minimum')
        ...
        local.set_title(f"{best['epoch_tdb']} TDB\n{interval_label}", fontsize=9)
    ...
    for i, m in enumerate(matches[:4]):
        ...
        ax.plot(x, y, 'o', mfc='none', mec='lime', ms=12, mew=1.2, label='measured centroid')
        ax.plot(m['predicted_x_px'], m['predicted_y_px'], '+', color='#ff40c8', ms=12, mew=1.5, label='joint candidate prediction')
        reference = comparison.get(str(m['detection_id']))
        title = f"{m['planet']} — source {m['detection_id']}\njoint: {m['separation_arcmin']:.2f} arcmin"
        if reference:
            ax.plot(reference['metadata_predicted_x_px'], reference['metadata_predicted_y_px'], 'x',
                    color='cyan', ms=10, mew=1.3, label='metadata-time prediction')
            title += f"\nmetadata: {reference['metadata_residual_arcmin']:.2f} arcmin"
        ax.set_xlim(x-14, x+14); ax.set_ylim(y+14, y-14)
        ax.set_title(title, fontsize=9)
        ax.set_xlabel('Detector x (pixels)', fontsize=8)
        if i == 0:
            ax.set_ylabel('Detector y (pixels)', fontsize=8)
            ax.legend(fontsize=6, loc='lower left', facecolor='white', framealpha=.8)
        ax.tick_params(labelsize=7)
    footer = fig.add_subplot(grid[2, :]); footer.axis('off')
    notes = [joint['reason'], 'All symbols are at actual positions; crops magnify the image, not residual vectors.']
    if best:
        ...
        if joint.get('metadata_comparison'):
            ...
            notes.append(f'After-fit comparison: candidate minus metadata = {offset/3600:+.3f} hours. Metadata did not bound or select the fit.')
