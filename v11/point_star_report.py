...
def _joint_display_planets(result, science):
    """Joint evidence has visual priority; timestamp matches stay validation."""
    ...
    return planet_product_view(planets, joint)
...
def report_sections(result, science=None, **legacy):
    ...
    detector_parity = result['camera'].get('detector_parity', 'normal')
...
def table_rows(result, science):
    ...
    if joint:
        rows.append(('Coordinate epoch', f"J{_fmt(result.get('coordinate_epoch_jyear'), 6)}; "
                     + ('joint model' if joint.get('adopted') else 'initial stellar-only model')))
...
def formula_page():
    ...
    right.text(.02, .350, r'$\boldsymbol{s}_{\rm ICRS}='
               r'\mathbf{Q}\mathbf{P}\boldsymbol{s}_{\rm local}$',
               fontsize=11, va='top')
    right.text(.02, .295,
               r'$\mathbf{P}=\operatorname{diag}(1,p,1)$ is the detector-parity reflection; '
               r'$p=+1$ normal, $p=-1$ mirrored.',
               fontsize=8.5, va='top', wrap=True)

    right.text(0, .225, 'Parameter roles', fontsize=12,
               weight='bold', va='top')
    roles = [
        ...
        (r'$\mathbf{Q},\mathbf{P}$', 'camera rotation; detector reflection'),
...
def photometry_page(output):
    ...
    for ax, channel in zip(axes, 'RGB'):
        ...
        ax.set_xlabel(f'{band} catalogue magnitude [mag]', fontsize=9)
        ax.set_ylabel(f'Camera {channel} instrumental magnitude [mag]', fontsize=9)
        ax.grid(alpha=.2)
...
def write_report(output, result, *, science=None, **unused):
    ...
    with PdfPages(path, metadata=metadata_pdf) as pdf:
        ...
        if science.get('joint_epoch') or result.get('joint_epoch'):
            ...
            joint_page = joint_figure(output, result, science)
            pdf.savefig(joint_page)
            save_png(joint_page, output/'joint_epoch_comparison.png', dpi=160)
            plt.close(joint_page)
    plt.close(fig)
    plt.close(reverse)
