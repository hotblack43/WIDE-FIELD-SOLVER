...
BRIGHT_PLANETS = ('mercury', 'venus', 'mars', 'jupiter', 'saturn', 'ceres', 'vesta')
SOURCE = ('Mallama & Hilton 2018 for major planets; bundled JPL Horizons '
          'quantity 9 apparent V magnitudes for Ceres and Vesta; Saturn globe only')
...
def planet_brightness(name, jd_tdb):
    ...
    if not len(dates):
        return dates.copy()
    if name in ('ceres', 'vesta'):
        from point_star_planet_ephemeris import minor_planet_magnitudes
        return minor_planet_magnitudes(name, dates)
