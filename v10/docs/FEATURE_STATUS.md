# Point-source feature status, version 0.10.0
...
|---|---|---|
| Direct scientific Canon CR2 input | Added in v0.10.0 | rawpy/LibRaw reads `raw_image_visible`, never the embedded JPEG. Supported 2×2 Bayer mosaics become native half-resolution `R/G1/G2/B` planes with correct colour-indexed black/white levels. Exposure, ISO, camera and lens are recorded pre-fit; capture time is requested only by the post-fit metadata stage. Regression: `test_point_star_raw.py`, `test_point_star_image.py`, `test_point_star_metadata.py` |
| Sequential multi-image launcher | Added in v0.10.0 | `go10.sh` and `go_v0.10.0.sh` accept multiple paths, including shell-expanded wildcards, and apply shared input/result options to every image. All inputs are validated before analysis; per-image failures retain their run data without stopping later inputs, and produce a nonzero batch exit. Regression: root `test_v10_launchers.py`; no scientific or numerical changes |
| Sky-domain WCS export | Added in v0.10.0 | The saved image-only footprint bounds ZPN validation when detector corners lie beyond the invertible sky. The 0.05-pixel limit is unchanged. Plain FITS pixels outside sky are NaN; annotated native planes are unchanged and a WCS-bearing `SKYMASK` is published. Regression: `test_point_star_fits.py` |
...
| Native-depth raster, stacked FITS and CR2 input | Extended in v0.10.0 | One authoritative loader preserves 8/12/14/16-bit PNG/TIFF samples, FITS physical values and native CR2 Bayer values for detection, photometry, planet evidence and export. It accepts 2-D monochrome, RGB and `R/G1/G2/B` stacks in either axis order or named extensions; G1/G2 remain separately measured while G is their floating mean. `input_image.json` records checksum, dtype/bit-depth evidence, calibration authority and channel mapping. NaN/Inf pixels are masked. Plot stretches are display-only; CR2 is split, not demosaiced |
...
Single-planet date/identity aliases no longer select an adopted epoch after the
absence-evidence stage. They remain intact in the JSON and CSV candidate lists;
the final status is `planet_epoch_not_identifiable`, its selected matches and
epoch are empty, and no unmatched-planet constellation is projected onto the
image. The PDF nevertheless reports the strongest retained blind candidate's
date, local positional uncertainty and residual as explicitly unconfirmed
single-planet evidence, separately from any fixed-metadata match. This preserves
the temporal information without making an independently unsupported identity
or epoch claim. Conditional multi-planet candidates retain the measured and
predicted overlays.
