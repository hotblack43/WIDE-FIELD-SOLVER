#!/usr/bin/env bash
# Convenience launcher pinned to the Gaia-capable wide-field solver 0.9.0.
set -euo pipefail
...
case "$1" in
    -h|--help) usage; exit 0 ;;
    --version) echo "Wide-field go9 $expected_version (requires solver $expected_version)"; exit 0 ;;
esac
input="$1"
shift
solver_args=()
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --fits-hdu|--channel-order|--saturation-level)
            ...
            ;;
        *)
            echo "Unsupported go9 option: $1" >&2
            usage >&2
            exit 2
            ;;
    esac
...
# Resolve every runtime dependency inside the committed v9 directory.
solver="$repo/v9"
