# Gaia v0.9.0 wide-field solver

V9 consolidates the complete home v8 and work v8a feature set in an independent
runtime. Its parent packages are checksum-frozen; run folders and database rows
record this package's distinct source manifest. The v0.9.0 display change keeps
...
[shared guide for go.sh through go9.sh](../README.md).

Run `./go9.sh /path/to/image.jpg` from the
repository root. This directory contains v0.9.0 and its local Python modules,
Gaia catalogue, provenance, dependency lockfile, tests and scientific status.
No development worktree is required.

Every `go9.sh` analysis automatically
...
V9 accepts native-depth grayscale/RGB PNG and TIFF plus 2-D, RGB and
...
```sh
./go9.sh /path/to/image.fits --fits-hdu SCI --channel-order RG1G2B \
...
After the blind tetra3 bootstrap, v9 performs catalogue association and robust
...
By default, the v9 planetary stage uses an observation time found in an explicit
...
usable time, v9 falls back to the full blind search. Force that search with:

```sh
./go9.sh /path/to/image.fits --blind-planets
```
...
[../docs/v8a-runtime.json](../docs/v8a-runtime.json); new v9 development belongs
here.

V9 searches Ceres and Vesta alongside the seven inherited major planets using
...
V9 inherits v7's conservative recovery of a third or later planet that was initially
...
V9 inherits v7's blind detector-parity selection. A tetra3 seed may describe either a
...
`go9.sh` stores both run folders and `stars.sqlite` under the
...
```sh
./go9.sh image.fits.bz2 --results-dir /path/to/larger/disk/results
# Or for a shell/batch:
export WFS_RESULTS_DIR=/path/to/larger/disk/results
./go9.sh image.fits.bz2
```
