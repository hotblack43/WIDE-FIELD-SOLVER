#!/usr/bin/env bash
# Convenience launcher pinned to the Gaia-capable wide-field solver 0.4.1.
set -euo pipefail
...
if [[ ! -f "$solver/data/stars_gaia_dr3_g75.csv" ]]; then
    solver="$repo/.worktrees/zenith-v0.4.1"
fi
...
if [[ ! -x "$solver/analyse.sh" || ! -f "$catalogue" ]]; then
    ...
    echo "Use a checkout of feature/zenith-v0.4.1, or keep its worktree in .worktrees/zenith-v0.4.1." >&2
    exit 2
fi
