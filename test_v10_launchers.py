"""Exercise packaged v10 dispatch without a worktree or scientific dependencies."""
import ast
import json
import os
...
class V10LauncherTests(unittest.TestCase):
    def setUp(self):
    ...
    def test_package_version_interfaces_agree(self):
        project = tomllib.loads((ROOT/PACKAGE/'pyproject.toml').read_text())
        lock = tomllib.loads((ROOT/PACKAGE/'uv.lock').read_text())
        ...
        module = ast.parse((ROOT/PACKAGE/'point_star_barghini.py').read_text())
    ...
    def make_package(self, root, version=VERSION):
        ...
        for name in LAUNCHERS + (f'{PACKAGE}/run.sh',):
    ...
    def test_launcher_runs_local_v10_and_preserve_each_output(self):
    ...
    def test_results_location_config_environment_and_cli_precedence(self):
        with tempfile.TemporaryDirectory(prefix='solver outputs ') as tmp:
            ...
            for index, destination in enumerate(locations):
                ...
                command = [str(root/'go10.sh'), str(image)]
                if index == 2:
    ...
    def test_unavailable_configured_storage_fails_without_local_fallback(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            completed = subprocess.run([str(root/'go10.sh'), str(image)], capture_output=True, text=True)
    ...
    def test_help_version_and_invalid_input_need_no_dependencies(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            self.make_package(root)
            (root/PACKAGE/'analyse.sh').unlink()
            for launcher in LAUNCHERS:
