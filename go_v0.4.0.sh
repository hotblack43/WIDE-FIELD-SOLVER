#!/usr/bin/env bash
# Convenience launcher pinned to the Gaia-capable wide-field solver 0.4.0.
set -euo pipefail
...
if [[ ! -f "$solver/data/stars_gaia_dr3_g75.csv" ]]; then
    solver="$repo/.worktrees/gaia-v0.4"
fi
...
if [[ ! -x "$solver/analyse.sh" || ! -f "$catalogue" ]]; then
    ...
    echo "Use a checkout of feature/gaia-v0.4, or keep its worktree in .worktrees/gaia-v0.4." >&2
    exit 2
fi
