...
\begin{document}
\title{Blind astrometry and instrumental photometry of fisheye sky images}
%\subtitle{ - }
\author{Peter Thejll\inst{1}\corrauth{pth@dmi.dk} \and Chris Flynn\inst{2}}
\institute{
  Danish Meteorological Institute, Skt. Kjelds Plads 11,
...
validation across heterogeneous wide fields, although not every image contains
...
The stellar astrometric fit uses only image-derived source positions; site, time,
camera, and object names are withheld. Every detected source remains available,
including large and saturated detections. Metadata may be revealed only after
astrometric parameters are fixed, for separate validation. A planet matched at a FITS
