...
def resolve_names(star_ids, *, cache_path=None, offline=False):
    ...
    for star_id in names.keys() & cached.keys():
        names[star_id] = cached[star_id]
    missing = [star_id for star_id in names if star_id not in cached]
    ...
    try:
        ...
        if result is not None:
            for row in result:
                ...
                names[star_id] = dict(display_name=choose_display_name(aliases+'|'+main_id, star_id),
