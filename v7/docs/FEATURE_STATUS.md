# Point-source feature status, version 0.7.0
...
| Automatic SQLite run storage | Inherited from working v6 | Every go7 analysis appends all available text products and CSV rows, including reruns, saturated/unusable sources, and partial failed runs. No deduplication or quality cut; never read by the solver. See [DATABASE.md](DATABASE.md) |
