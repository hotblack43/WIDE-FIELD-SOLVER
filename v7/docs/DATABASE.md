...
Run `./go7.sh /full/path/to/image.jpg` as usual. The launcher automatically passes
`--database REPOSITORY/results/stars.sqlite` to the analysis process. The database
...
The lower-level `v7/analyse.sh` accepts `--database PATH` for explicit callers.
