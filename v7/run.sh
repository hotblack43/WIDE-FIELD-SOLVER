#!/usr/bin/env bash
# Convenience launcher pinned to the Gaia-capable wide-field solver 0.7.0.
set -euo pipefail
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --fits-hdu|--channel-order|--saturation-level)
            ...
            ;;
        *)
            echo "Unsupported go7 option: $1" >&2
            usage >&2
            exit 2
            ;;
    esac
...
# Resolve every runtime dependency inside the committed v7 directory.
solver="$repo/v7"
