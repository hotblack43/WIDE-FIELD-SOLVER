"""Exercise packaged v8a dispatch without a worktree or scientific dependencies."""
import ast
import json
import os
...
class V8aLauncherTests(unittest.TestCase):
    def setUp(self):
    ...
    def test_package_version_interfaces_agree(self):
        project = tomllib.loads((ROOT/'v8a/pyproject.toml').read_text())
        lock = tomllib.loads((ROOT/'v8a/uv.lock').read_text())
        ...
        module = ast.parse((ROOT/'v8a/point_star_barghini.py').read_text())
    ...
    def make_package(self, root, version=VERSION):
        (root/'v8a/data').mkdir(parents=True)
        for name in LAUNCHERS + ('v8a/run.sh',):
            destination = root/name
            destination.parent.mkdir(parents=True, exist_ok=True)
            shutil.copy2(ROOT/name, destination)
        (root/'v8a/data/stars_gaia_dr3_g75.csv').write_text('fixture')
        stub = root/'v8a/analyse.sh'
    ...
    def test_launcher_runs_local_v8a_and_preserve_each_output(self):
        with tempfile.TemporaryDirectory(prefix='solver package ') as tmp:
            ...
            for launcher in LAUNCHERS:
                for _ in range(2):
                    ...
                    self.assertIn('Solver: Wide-field solver 0.8.0, Gaia DR3', completed.stdout)
            runs = sorted((root/'results/runs').iterdir())
            self.assertEqual(len(runs), 2)
            for run in runs:
                self.assertIn('-v0.8.0-', run.name)
                args = json.loads((run/'analysis/args.json').read_text())
                self.assertEqual(args, [str(image), '--catalog',
                                       str(root/'v8a/data/stars_gaia_dr3_g75.csv'),
    ...
    def test_results_location_config_environment_and_cli_precedence(self):
        with tempfile.TemporaryDirectory(prefix='solver outputs ') as tmp:
            ...
            for index, destination in enumerate(locations):
                ...
                command = [str(root/'go8a.sh'), str(image)]
                if index == 2:
    ...
    def test_unavailable_configured_storage_fails_without_local_fallback(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            completed = subprocess.run([str(root/'go8a.sh'), str(image)], capture_output=True, text=True)
