"""Exercise packaged v5 dispatch without a worktree or scientific dependencies."""
import ast
import json
...
LAUNCHERS = ('go5.sh', 'go_v0.5.0.sh')


class V5LauncherTests(unittest.TestCase):
    def test_package_version_interfaces_agree(self):
        project = tomllib.loads((ROOT/'v5/pyproject.toml').read_text())
        lock = tomllib.loads((ROOT/'v5/uv.lock').read_text())
        self.assertEqual(project['project']['version'], '0.5.0')
        self.assertEqual(next(p['version'] for p in lock['package']
                              if p['name'] == project['project']['name']), '0.5.0')
        module = ast.parse((ROOT/'v5/point_star_barghini.py').read_text())
    ...
    def make_package(self, root, version='0.5.0'):
        (root/'v5/data').mkdir(parents=True)
        for name in LAUNCHERS + ('v5/run.sh',):
            shutil.copy2(ROOT/name, root/name)
        (root/'v5/data/stars_gaia_dr3_g75.csv').write_text('fixture')
        stub = root/'v5/analyse.sh'
    ...
    def test_both_launchers_run_local_v5_and_preserve_each_output(self):
        with tempfile.TemporaryDirectory(prefix='solver package ') as tmp:
            ...
            for launcher in LAUNCHERS:
                for _ in range(2):
                    ...
                    self.assertIn('Solver: Wide-field solver 0.5.0, Gaia DR3', completed.stdout)
            runs = sorted((root/'results/runs').iterdir())
            self.assertEqual(len(runs), 4)
            for run in runs:
                self.assertIn('-v0.5.0-', run.name)
                args = json.loads((run/'analysis/args.json').read_text())
                self.assertEqual(args, [str(image), '--catalog',
                                       str(root/'v5/data/stars_gaia_dr3_g75.csv'),
    ...
    def test_help_version_and_invalid_input_need_no_dependencies(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            self.make_package(root)
            (root/'v5/analyse.sh').unlink()
            for launcher in LAUNCHERS:
                for option, fragment in (('--version', '0.5.0'), ('--help', 'Usage:')):
    ...
    def test_version_mismatch_refuses_analysis(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            self.make_package(root, version='0.4.3')
