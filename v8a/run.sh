#!/usr/bin/env bash
# Convenience launcher pinned to the Gaia-capable wide-field solver 0.8.0.
set -euo pipefail
...
usage() {
    echo "Usage: $0 /full/path/to/image.{jpg,png,tiff,fits,fits.bz2} [native-input options]"
...
case "$1" in
    -h|--help) usage; exit 0 ;;
    --version) echo "Wide-field go8a $expected_version (requires solver $expected_version)"; exit 0 ;;
esac
input="$1"
shift
solver_args=()
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --fits-hdu|--channel-order|--saturation-level)
            ...
            ;;
        *)
            echo "Unsupported go8a option: $1" >&2
            usage >&2
            exit 2
            ;;
    esac
...
# Resolve every runtime dependency inside the committed v8a directory.
solver="$repo/v8a"
...
if ! run_dir="$(mktemp -d "$results_root/runs/${stem:-image}-v${expected_version}-v8a-$(date -u +%Y%m%dT%H%M%SZ)-XXXXXX")"; then
    echo "Cannot create a run in $results_root; use --results-dir on a filesystem with free space." >&2
    exit 1
fi
output="$run_dir/analysis"

echo "Solver: $actual_version, Gaia DR3 (v8a work snapshot)"
echo "Image: $image"
