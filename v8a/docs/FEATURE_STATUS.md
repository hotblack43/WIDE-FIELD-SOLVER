# Point-source feature status, version 0.8.0
...
|---|---|---|
| Portable results storage | Added in v0.8.0 | The `go8a.sh` launcher supports `--results-dir`, `WFS_RESULTS_DIR` and a host-specific user preference; run folders and SQLite stay together. Other machines retain the repository-local default. Missing configured storage fails explicitly. Regression: root `test_v8a_launchers.py`; no numerical changes |
...
| Automatic SQLite run storage | Inherited from working v6 | Every go8a analysis appends all available text products and CSV rows, including reruns, saturated/unusable sources, and partial failed runs. No deduplication or quality cut; never read by the solver. See [DATABASE.md](DATABASE.md) |
