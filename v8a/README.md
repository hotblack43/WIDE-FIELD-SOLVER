# Gaia v8a work snapshot

Independent copy of the work machine's v8 on 2026-09-18. The scientific
version remains 0.8.0; run folders include `v8a`, and the database records this
package's distinct source manifest. See [HANDOFF.md](HANDOFF.md) before pushing
or pulling between computers. No scientific algorithms changed in this copy.

For installation, choosing a solver and running images, use the
[shared guide for go.sh through go8.sh](../README.md).

Run `./go8a.sh /path/to/image.jpg` from the
...
Every `go8a.sh` analysis automatically
...
```sh
./go8a.sh /path/to/image.fits --fits-hdu SCI --channel-order RG1G2B \
...
```sh
./go8a.sh /path/to/image.fits --blind-planets
```
...
[../docs/v7-runtime.json](../docs/v7-runtime.json); new v8a development belongs
here.
...
```sh
./go8a.sh /path/to/image.fits.bz2
```
...
`go8a.sh` store both run folders and `stars.sqlite` under the
...
```sh
./go8a.sh image.fits.bz2 --results-dir /path/to/larger/disk/results
# Or for a shell/batch:
export WFS_RESULTS_DIR=/path/to/larger/disk/results
./go8a.sh image.fits.bz2
```
