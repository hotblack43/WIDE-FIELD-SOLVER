...
| Automatic SQLite run storage | Inherited from working v6 | Every go8 analysis appends all available text products and CSV rows, including reruns, saturated/unusable sources, and partial failed runs. No deduplication or quality cut; never read by the solver. See [DATABASE.md](DATABASE.md) |
