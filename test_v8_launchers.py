"""Exercise packaged v8 dispatch without a worktree or scientific dependencies."""
import ast
import json
...
LAUNCHERS = ('go8.sh', 'go_v0.8.0.sh')
VERSION = '0.8.0'


class V8LauncherTests(unittest.TestCase):
    def test_package_version_interfaces_agree(self):
        project = tomllib.loads((ROOT/'v8/pyproject.toml').read_text())
        lock = tomllib.loads((ROOT/'v8/uv.lock').read_text())
        ...
        module = ast.parse((ROOT/'v8/point_star_barghini.py').read_text())
    ...
    def make_package(self, root, version=VERSION):
        (root/'v8/data').mkdir(parents=True)
        for name in LAUNCHERS + ('v8/run.sh',):
            destination = root/name
            destination.parent.mkdir(parents=True, exist_ok=True)
            shutil.copy2(ROOT/name, destination)
        (root/'v8/data/stars_gaia_dr3_g75.csv').write_text('fixture')
        stub = root/'v8/analyse.sh'
    ...
    def test_both_launchers_run_local_v8_and_preserve_each_output(self):
        with tempfile.TemporaryDirectory(prefix='solver package ') as tmp:
            ...
            for run in runs:
                ...
                self.assertEqual(args, [str(image), '--catalog',
                                       str(root/'v8/data/stars_gaia_dr3_g75.csv'),
